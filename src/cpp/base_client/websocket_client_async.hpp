...
public:
    explicit WebsocketClient(WebsocketCallbacks &callbacks, std::string &sessionIdenfitier, net::io_context &ioc, ssl::context &ctx, std::string host, std::string port, char const *target)
        : callbacks_(callbacks), sessionIdenfitier_(sessionIdenfitier), ioc_(ioc), ctx_(ctx), resolver_(ioc_), host_(host), port_(port), handshake_target_(target) {}
...
private:
    void on_resolve(beast::error_code ec, tcp::resolver::results_type results) {
        ...
        net::async_connect(get_lowest_layer(*ws_), results.begin(), results.end(), beast::bind_front_handler(&WebsocketClient::on_connect, shared_from_this()));
    }
    ...
    void on_ssl_handshake(beast::error_code ec) {
        ...
        ws_->async_handshake(host_ + ':' + port_, handshake_target_, beast::bind_front_handler(&WebsocketClient::on_handshake, shared_from_this()));
    }
